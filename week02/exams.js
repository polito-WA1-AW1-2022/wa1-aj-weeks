...
function ExamList() {
    ...
    this.afterDate = (startDate) => {
        const result = new ExamList() ;
        const startDay = dayjs(startDate) ;
        result.examList = this.examList.filter( (exam) => exam.date.isSameOrAfter(startDay)) ;
        return result ;
    }
    ...
    //     return this.examList.map(exam => exam.score).reduce((partial, current)=>partial+current, 0) / this.examList.length
    // }
